...
class SajdaAyatsModel {
  int? code;
  String? status;
  Data? data;

  SajdaAyatsModel({this.code, this.status, this.data});

  SajdaAyatsModel.fromJson(Map<String, dynamic> json) {
    code = json['code'];
    status = json['status'];
...
Future<SajdaAyatsModel> fetchSajdaAyats() async {
  final url = Uri.parse(Keys.getAllSajdaAyatsEndPoint);

  try {
    final response = await http.get(url);
    if (response.statusCode == 200) {
      final jsonResponse = json.decode(response.body);
      return SajdaAyatsModel.fromJson(jsonResponse);
    } else {
